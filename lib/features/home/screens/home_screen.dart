  ...
  Future<void> loadData({bool isReload = false}) async {
    ...
    if (Get.find<RideController>().rideDetails != null) {
      ...
      if (Get.find<RideController>().rideDetails?.currentStatus == 'ongoing') {
        if (Get.find<RideController>().rideDetails?.customerSafetyAlert !=
            null) {
          Get.find<SafetyAlertController>()
              .updateSafetyAlertState(SafetyAlertState.afterSendAlert);
        } else {
          Get.find<RideController>()
              .remainingDistance(Get.find<RideController>().rideDetails!.id!);
          Get.find<SafetyAlertController>().checkDriverNeedSafety();
        }
      }

      PusherHelper()
          .pusherDriverStatus(Get.find<RideController>().rideDetails!.id!);
    } else {
              ...
              ? Padding(
                  padding: EdgeInsets.only(
                      bottom: rideController.biddingList.isEmpty &&
                              (HomeScreenHelper.getRideCount() == 0)
                          ? Get.height * 0.08
                          : 0),
                  child: JustTheTooltip(
                    ...
                    child: InkWell(
                      onTap: () => Get.to(() => const OngoingParcelListView(
          ...
          rideController.biddingList.isNotEmpty && _isShowRideIcon
              ? Padding(
                  ...
                  child: InkWell(
                    ...
                    child:
                        Image.asset(Images.biddingIcon, height: 60, width: 60),
                  ),
                )
              : const SizedBox()
        ]);
