...
class _EditProfileAccountInfoState extends State<EditProfileAccountInfo> {
  ...
  void initState() {
    ...
    idNumberController.text = Get.find<ProfileController>()
            .profileModel!
            .data!
            .identificationNumber ??
        '';
    if (Get.find<ProfileController>().profileModel!.data != null) {
      Get.find<ProfileController>().setIdentityType(
      ...
      Get.find<ProfileController>().setIdentityType(
          Get.find<ProfileController>().identityTypeList[0],
          notify: false);
    }
  }
        ...
        Expanded(
          ...
          child:
            ...
            if (profileController
                .profileModel!.data!.identificationImage!.isNotEmpty)
              Padding(
                ...
                child: ListView.builder(
                  ...
                  itemCount: profileController
                      .profileModel?.data?.identificationImage?.length,
                        ...
                        child: Stack(children: [
                          ClipRRect(
                            ...
                            child: SizedBox(
                              ...
                              child: ImageWidget(
                                  image:
                                      "${Get.find<ConfigController>().config?.imageBaseUrl?.identityImage}/${profileController.profileModel?.data?.identificationImage?[index] ?? ''}"),
                            ),
                          ),
                          Positioned(
                            bottom: 0,
            ...
            Padding(
              ...
              child: ListView.builder(
                ...
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemCount: profileController.identityImages.length >= 2
                    ? 2
                    : profileController.identityImages.length + 1,
                itemBuilder: (BuildContext context, index) {
                  return index == profileController.identityImages.length
                      ? InkWell(
                          onTap: () async {
                            bool res = await profileController.pickImage(
                              false,
                              false,
                            );
                            if (res) {
                      ...
                      : Stack(children: [
                          ...
                          Positioned(
                            ...
                            right: 0,
                            child: InkWell(
                              onTap: () => profileController.removeImage(index),
                              child: Container(
                                decoration: const BoxDecoration(
                                  color: Colors.white,
            ...
            : ButtonWidget(
                ...
                onPressed: () async {
                  if (idNumberController.text.isEmpty ||
                      idNumberController.text.length < 6 ||
                      idNumberController.text.length > 50) {
                    showCustomSnackBar('identity_number_is_invalid'.tr,
                        isError: true);
                  } else {
                    Response response = await profileController.updateProfile(
                      firstNameController.text,
                      lastNameController.text,
                      profileController.identityType,
